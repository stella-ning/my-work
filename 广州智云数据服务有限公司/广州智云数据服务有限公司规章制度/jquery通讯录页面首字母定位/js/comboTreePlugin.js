(function ($, window, document, undefined) {
    ...
    ComboTree.prototype.initevent = function () {
        ...
        $(this.element).on('click', function (event) {
            ...
            if (($(event.target).hasClass('imitationSelect') || $(event.target).hasClass('select-tree-wrap')||$(event.target).hasClass('input-tips')||$(event.target).attr('data-parentId')||$(event.target).hasClass('fa')) && closet.length > 0) {
            ...
            if (target) {
                ...
                if (target.attr('role') !== 'parent') {
                    ...
                    let parentId = target.attr("data-parentId");
                    let parentName = target.attr("data-parentName");
                    ...
                    if (target.hasClass('actived_li')) {
                        ...
                        _this.checkrow(target, oliId,parentId, parentName,false); //第三个参数表示点击的是否是checkbox
                    }
            ...
            if ($(event.target).attr('type') === 'checkbox') {
                ...
                if (_target.hasClass('actived_li')) {
                    ...
                    _this.checkrow(_target, $(event.target).attr('data-id'),parentId,parentName, true);
                }
            }
    ...
    ComboTree.prototype.initdom = function () {
        $(this.element).append(
            ...
            '<span _id="myplaceholder" class="input-tips">请选择部门</span>'+
            '</div>' +
    ...
    ComboTree.prototype.createitem = function (SampleJSONData, container) {
        for (let j = 0; j < SampleJSONData.length; j++) {
            ...
            var parentId=SampleJSONData[j].parentId;
            var parentName=SampleJSONData[j].parentName;
            
            // li容器
            let item = $('<li data-name="' + oliName + '" data-id="' + oliId + '" data-parentId="' + parentId+'" data-parentName="' + parentName+ '"></li>'),
                divitem = $('<div data-name="' + oliName + '" data-id="' + oliId + '" data-parentId="' + parentId+'" data-parentName="' + parentName+'" class="title-container"></div>');
    ...
    ComboTree.prototype.checkrow = function (target, oliId, parentId,parentName,ischeckbox) {
        let _this = this;
        ...
        let item = $("<span data-id='" + oliId + "' class='input-keyword-item'></span>"),
            namespan = $("<span data-parentId="+parentId+" data-parentName="+parentName+">" +target.attr('data-parentName')+"--"+ target.attr('data-name') + "</span>"),
